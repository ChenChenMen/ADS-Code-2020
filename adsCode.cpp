//Active Drag Code, Coded by Taber Fisher and Salim Roustom for the Rocketry at Virginia Tech Design Team
...
const double apogeeThreshold = 1; //in Gs
...
const double pGROUND = 29.91 * 3386.39; //Ground pressure (convert from in Hg to Pa)
const double TGROUND = 295.15; // ground temperature, K (313.706 K = 105 F, 310.928 K = 100 F)
...
const double onePLUSgMoverRL0 = 1 + (g*M / R / L0);
...
int main() {
  //Runs the required setup
  setup();

  //Initializes the SD file
  printString("Main");
...
//Function that will be run once before the main function
void setup() {
  //Arduino Setup
  Serial.begin(115200);
  ...
  //NEEDS TO BE UPDATED FOR BEAGLEBONE

  Serial.println("SDStart");
  ...
  Serial.println("SDGood");

  //IMU Setup -------------------------------------------------------------------------------
  Wire.begin();

  if (!imu.init()) {
    Serial.println("IMUFailed");
    while (1);
  }
  Serial.println("IMUGood");
  imu.enableDefault();
  ps.enableDefault();
  ...
  myServo.write(60);
  myServo.write(62); // Keeps the servo at the minimum actuation in the rocket without straining the servo

  File testFile = SD.open(fileName, FILE_WRITE);
  testFile.println("Top");
  testFile.close();
...
/*
  ...
  @param: String: A string that will be written to the SD card
*/
...
/*
  ...
  @param: int: Integer to write to the SD card
*/
...
/*
  ...
  @param: int: Double to write to the SD card
*/
...
/*
  This method detects the launch of the rocket through a threshold value and reads
  data while waiting
*/
void detectLaunch() {
  imu.read();
...
/*
  This method dectects burnout of the rocket motor either through an acceleration
  threshold or through a time threshold
  @param: double: the time that the rocket launched
*/
void detectBurnout(double launchTime) {
  ...
  if (imu.a.z > (burnoutThreshold / gConversionFactor)) {
    printString("aBurnout");
  } else {
    printString("tBurnout");
  }
}
...
*/
bool detectApogee(double burnoutTime) {
  readIMU();

  if ((imu.a.z >= (apogeeThreshold / gConversionFactor)) || ((millis() - burnoutTime) > 50000)){
    printString("Apogee");
    printString("EndFile");
    myServo.write(65);
    return true;
...
/*
  This function predicts the expeced apogee of the rocket and the CD necessary
  to actuate the ADS to
  @return: What the CD needs to be changed to
*/
double prediction() {
  ...
  int minId = 0;
  for (int i = 1; i <= 4; i++) {
    if ( abs(finalResults[i] - 3048) < minDiff) {
      minId = i;
    }
  }

  double startP = (minId * .25) - .1;
  double specificResults[5];
  for (int i = 0; i <= 4; i ++) {
    double currP = startP + (0.05 * i);
  ...
  int sPred = ((startP + (0.05 * minId)) * 50) + 60;
  if ((sPred > 60) && (sPred < 110)) {
    myServo.write(sPred);
  }

  printString("The Servo needs to be actuated to: ");
  printInt(sPred);
  return (sPred);
}

/*
  The function that solves the coupled equations, this uses runge-kutta
  @param: double: the timestep that you want the ode to be solved at
  @param: double: what flap percent that the rocket has currently
...
void ODE(double t, double Y[], double Y_p[]) {
  ...
  Y_p[1] = Y[3] * sin(Y[2]);
  Y_p[2] = -g * cos(Y[2]) / Y[3];
  //See if we can make this multiple lines, for redablitiy
  Y_p[3] = (negativeoneover2m0 * (rhoGROUND * pow((1 - L0 * Y[1] / TGROUND), onePLUSgMoverRL0))
            * pow(Y[3], 2)) * (((30.8451 * 0.00064516) * (0.445623145893995 + -0.290001570655917 * Y[3]
                                / sqrt(R * gamma * (TGROUND + L0 * Y[1])) + 0.561689194232028 * pow(Y[3], 2) / R / gamma /
                                (TGROUND + L0 * Y[1]))) + flapPercentCD) - (g * sin(Y[2]));
}

/*
  Takes data from to sensors and applies a complementary filter to make sure
  that we get more accurate data
*/
void complementaryFilter() {
  ...
  Y_current[3] = (.75 * getBaroV()) + (.25 * getAccV());
...
/*
  A function that get velocity from barameter
  @return: the velocity from the barameter
*/
double getBaroV() {
  ...
  long dtx = 10; //two times of the time step between ode runs
  double baroV = sqrt( pow(((secondAlt - firstAlt) / dt / 1000), 2) + pow(((secondX - firstX) / dtx / 1000), 2));
  printString("bV");
  printDouble(baroV);
  return (baroV);
}
...
/*
  A function that gets the velocity from the altimeter
  @return: the velocity from alimeter
*/
double getAccV() {
  long firstTime = millis();
  imu.read();
  ...
  double accelV = sqrt( pow(vXY, 2) + pow(vZ, 2));
  printString("aV");
  printDouble(accelV);
  return (accelV);
}
